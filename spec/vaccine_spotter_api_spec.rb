...
describe VaccineSpotterApi do
  ...
  describe "#find_in" do
    ...
    it "calls the API to the correct state" do
      ...
      VaccineSpotterApi.find_in(state: "IL")
    end

    VACCINE_TYPES.each do |vaccine_type|
      it "returns appointments for #{vaccine_type}" do
        locations = VaccineSpotterApi.find_in(state: "IL", vaccine_type: vaccine_type)
    ...
    it "filters the results by zipcode" do
      locations = VaccineSpotterApi.find_in(state: "IL", zipcodes: ["60601"])

      expect(locations).to be_empty

      locations = VaccineSpotterApi.find_in(state: "IL", zipcodes: ["60453"])
